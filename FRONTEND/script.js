...
async function calcularImportacion() {
  ...
  try {
    ...
    // ===============================
    // VALORES DEL BACKEND
    // Mapeados según la respuesta real del backend
    // ===============================
    const impMat        = data.impuesto_matriculacion?.importe ?? 0;
    const porcMat       = data.impuesto_matriculacion?.porcentaje ?? 0;
    const baseMat       = data.base_matriculacion ?? 0;
    const valorVenal    = data.valor_venal_depreciado ?? 0;
    const valorBaseBOE  = data.modelo_boe?.valor ?? 0;
    const precioSinIVA  = data.precio_sin_iva ?? 0;

    const iva           = data.iva_y_aranceles?.iva_importe ?? 0;
    const itp           = data.itp?.importe ?? 0;
    const itv           = data.costos_fijos?.itv_importacion ?? 0;
    const matriculacion = data.costos_fijos?.tasa_dgt ?? 0;
    const placas        = data.costos_fijos?.placas_verdes ?? 0;
    const matriculas    = data.costos_fijos?.matriculas ?? 0;
    const gestoria      = data.costos_fijos?.gestoria ?? 0;
    const total         = data.total_final ?? impMat;
    ...
    const ivaVisual = (ivaIncluido === "margen" || ivaIncluido === "si" || tipoCompra === "particular") ? 0 : iva;
    document.getElementById("f_iva_incluido").textContent = formato(ivaVisual);
    ...
    if (tipoCompra === "particular") {
      ...
      textoIVA = `
        <p><strong>IVA (21%) no incluido en origen</strong></p>
        <p>Se aplica el 21% sobre el precio sin IVA.</p>
         <p>Base: ${formato(precioSinIVA)} × 21% = <strong>${formato(iva)}</strong></p>
        `;
    ...
    // Actualizar título del chip de IVA
    const titulosIVA = {
        "particular": "IVA (venta particular)",
        "si": "IVA (incluido en origen)",
        "no": "IVA (21%)",
       "margen": "IVA (régimen de margen)"
    };
    const claveIVA = tipoCompra === "particular" ? "particular" : ivaIncluido;
    document.getElementById("titulo_iva").textContent = titulosIVA[claveIVA] || "IVA";
    ...
    if (data.itp?.aplicable) {
      textoITP = `
        ...
        <p>Tipo: ${((data.itp.porcentaje ?? 0) * 100).toFixed(2)}%</p>
    ...
    const porcentaje = porcMat * 100;
    const coefDepreciacion = valorBaseBOE > 0 ? (valorVenal / valorBaseBOE) : 1;

    let textoMatriculacion = "";

    if (precioSinIVA > valorVenal) {
      textoMatriculacion = `
        <p><strong>Impuesto de Matriculación (según CO₂)</strong></p>
        <p>Emisiones declaradas: <strong>${co2} g/km</strong></p>
        <hr style="border:none;border-top:1px solid var(--border);margin:6px 0;">
        <p><strong>Depreciación por antigüedad:</strong></p>
        <p>Valor venal BOE: ${formato(valorBaseBOE)} × coef. ${coefDepreciacion.toFixed(2)} = <strong>${formato(valorVenal)}</strong></p>
        <p>Precio pagado sin IVA: <strong>${formato(precioSinIVA)}</strong></p>
        <p>Se usa el <strong>precio sin IVA</strong> por ser superior al valor venal depreciado.</p>
        <hr style="border:none;border-top:1px solid var(--border);margin:6px 0;">
        <p><strong>Base utilizada:</strong> ${formato(baseMat)}</p>
      ...
      textoMatriculacion = `
        ...
        <p>Valor venal BOE: ${formato(valorBaseBOE)} × coef. ${coefDepreciacion.toFixed(2)} = <strong>${formato(valorVenal)}</strong></p>
        <p>Precio pagado sin IVA: <strong>${formato(precioSinIVA)}</strong></p>
        <p>Se usa el <strong>valor venal depreciado</strong> por ser superior al precio sin IVA.</p>
        <hr style="border:none;border-top:1px solid var(--border);margin:6px 0;">
        <p><strong>Base utilizada:</strong> ${formato(baseMat)}</p>
        <p><strong>Tipo aplicable:</strong> ${porcentaje.toFixed(2)}%</p>
        <p><strong>Importe final:</strong> ${formato(impMat)}</p>
...
async function recalcIVTM() {
  ...
  if (msg) msg.textContent = `IVTM estimado: ${ivtm.importe} €`;
}
