...
async function calcularImportacion() {
  ...
  try {
    ...
    const baseMat       = data.valor_venal ?? 0;
    const valorVenal    = data.valor_venal ?? 0;
    const precioSinIVA  = data.precio_base ?? 0;
    ...
    if (precioSinIVA > valorVenal) {
      textoMatriculacion = `
        ...
        <p>Valor venal depreciado: <strong>${formato(valorVenal)}</strong></p>
