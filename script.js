...
async function calcularImportacion() {
  ...
  /* DESGLOSES */
  document.getElementById("d_precio").innerHTML = `Precio declarado: ${formato(precio)}`;
  document.getElementById("d_iva").innerHTML = `IVA aplicado: ${formato(iva)}`;
  document.getElementById("d_itp").innerHTML = `
    Tipo aplicado: 4%<br>
    Tipo de compra: ${tipoCompra}<br>
    Base imponible: ${formato(precio)}<br>
    Importe: ${formato(itp)}
  `;
  document.getElementById("d_itv").innerHTML = `ITV importación: ${formato(itv)}`;
  document.getElementById("d_matriculacion").innerHTML = `Tasas DGT: ${formato(matriculacion)}`;
  document.getElementById("d_matriculacion_co2").innerHTML =
    `Tipo: ${tipoMat}%<br>CO₂: ${co2} g/km<br>Importe: ${formato(impMat)}`;

/* DESGLOSE IVTM (Circulación) */
let textoCirculacion = "";

// Obtener combustible del modelo seleccionado
const modeloBoe = coincidenciasGlobal.find(m => m.indice === modeloSeleccionado);
const combustible = modeloBoe?.combustible?.toUpperCase() || "";

// Clasificación
const esElectrico = combustible.includes("ELECT");
const esHibrido = combustible.includes("HIBRIDO");
const esHibridoEnchufable = combustible.includes("PHEV") || combustible.includes("ENCHUF");

// Texto dinámico según tipo de vehículo
if (esElectrico) {
  textoCirculacion = `
    Importe final: ${formato(circulacion)}<br>
    <em>Cálculo según potencia fiscal (vehículo eléctrico).</em>
  `;
} else if (esHibridoEnchufable) {
  textoCirculacion = `
    Importe final: ${formato(circulacion)}<br>
    <em>Cálculo según potencia fiscal (híbrido enchufable).</em>
  `;
} else if (esHibrido) {
  textoCirculacion = `
    Importe final: ${formato(circulacion)}<br>
    <em>Cálculo según potencia fiscal (híbrido no enchufable).</em>
  `;
} else {
  textoCirculacion = `
    Importe final: ${formato(circulacion)}<br>
    <em>Cálculo estimado según cilindrada.</em>
  `;
}
...
document.getElementById("d_circulacion").innerHTML = textoCirculacion;
  ...
  document.getElementById("d_placas").innerHTML = `Placas verdes: ${formato(placas)}`;
  document.getElementById("d_matriculas").innerHTML = `Matrículas: ${formato(matriculas)}`;
  document.getElementById("d_gestoria").innerHTML = `Gestoría: ${formato(gestoria)}`;
}
